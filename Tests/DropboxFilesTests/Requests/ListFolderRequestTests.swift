...
@Suite struct ListFolderTests {
	...
	@Test func encoding() throws {
		...
		#expect(urlRequest.url?.absoluteString == "https://api.dropboxapi.com/2/files/list_folder")
	...
	@Test func decoding() throws {
		...
		let request = ListFolder.Request(path: "", isRecursive: false)
		...
		#expect(response.entries[0] as? FileMetadata == FileMetadata(
	...
	@Test func decodingLookupFailure() throws {
		...
		let request = ListFolder.Request(path: "", isRecursive: false)
		#expect(throws: ListFolder.Error.lookup(.notFound)) {
			try request.response(from: data)
		}
	}

	@Test func decodingTemplateFailure() throws {
		let data = Data("""
		{
			"error_summary": "template_error/template_not_found/.",
			"error": {
				".tag": "template_error",
				"template_error": {
		...
		let request = ListFolder.Request(path: "", isRecursive: false)
		#expect(throws: ListFolder.Error.template(.notFound)) {
			try request.response(from: data)
		}
	}
